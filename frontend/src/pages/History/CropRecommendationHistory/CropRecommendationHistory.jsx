...
import { Card, CardContent, Typography, Button, Container, Box, LinearProgress } from "@mui/material";
import { motion } from "framer-motion";
import { useAuth } from "../../../services/authContext";
import axios from "axios";
import { useNavigate } from "react-router-dom"
function CropRecommendationHistory() {
  const { user } = useAuth();
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:4200/store-recommendation-crop/getbyuserID/${user.id}`
        );
        if (response) {
...
export default CropRecommendationHistory;